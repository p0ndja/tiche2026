    ...
    <div class="container mb-3">
        <div class="row">
            ...
            <div class="col-12">   
                ...
                <h2 class="font-weight-bold">High School Project Contest Submission</h2>
                    ...
                    <table class="table table-sm table-hover w-100 d-block d-md-table text-nowrap" id="attachmentTable">
                        ...
                        <tbody>
                            <?php
                                ...
                                if ($stmt = $conn->prepare('SELECT * FROM school_submission '.$extraCondition.' ORDER BY sub_timestamp DESC')) {
                                    $stmt->execute();
                                    $result = $stmt->get_result();
                                    if ($result->num_rows > 0) {
                                        while ($row = $result->fetch_assoc()) { 
                                            preg_match('/Project Name.*?: (.*)/', $row['data'], $matches);
                                            $projectName = $matches[1];
                                            ?>
                                        <tr onmouseup="window.location.href = '/submission-school/<?php echo $row['id']; ?>'" style="cursor: pointer">
                                        <td class="text-center"><a href="/submission-school/<?php echo $row['id']; ?>"><?php echo $row['id']; ?></a></td>
                                        <td class="text-left"><a href="/submission-school/<?php echo $row['id']; ?>"><?php echo $projectName; ?></a></td>
