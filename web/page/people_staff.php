...
<body>
    <div class="container-fluid px-5">
        <h1 class="font-weight-bold text-md">บุคลากรสายสนับสนุน</h1>
        <div class="row mb-3">
            <div class="col-md-3">
                <div class="card mb-3">
                    <ul class="nav md-pills pills-success flex-column" role="tablist">
                        <?php
                            $fol = glob("../file/people/staff/*", GLOB_ONLYDIR);
                            ...
                            <script>
                                function createNewCategory() {
                                    ...
                                    if (folderName != null && folderName != "") {
                                        window.location = "../endpoint/people_IO.php?function=create&top=staff&mkdir=" + folderName;
                                    }
                                }
                ...
                <div class="tab-content vertical">
                    <?php for($f = 0; $f < count($fol); $f++) {
                        ...
                        <div class="tab-pane fade in show <?php if ($f == 0) echo 'active'; ?>" id="<?php echo str_replace(" ", "_", pathinfo($fol[$f], PATHINFO_FILENAME)); ?>" role="tabpanel">
                            <?php $hotlink = "";
                            if (file_exists("../file/people/staff/$folder_name/link.txt")) {
                                $link = "";
                                foreach(readTxt("../file/people/staff/$folder_name/link.txt") as $l) {
                                    $link .= $l;
                                }
                                if (!empty($link) || isAdmin()) 
                                    $hotlink = "<a class=\"md\" ".(isAdmin() ? "href=\"#\" onclick='myEditLinkFunction(\"../file/people/staff/$folder_name\",\"".$link."\");'" : "href=\"".$link."\"")."><i class=\"fa-solid fa-link\"></i></a>";
                            } else if (isAdmin()) {
                                $hotlink = "<a class=\"md\" href=\"#\" onclick=\"myEditLinkFunction('../file/people/staff/$folder_name','');\"><i class=\"fa-solid fa-link\"></i></a>";
                            }?>
                            <h4 class="font-weight-bold"><?php echo $folder_name . " " . $hotlink; ?></h4>
                            <div class="row">
                            <?php
                                $file = glob("../file/people/staff/$folder_name/*.{jpg,png,gif,PNG,JPG,GIF,JPEG,jpeg}", GLOB_BRACE);
                                $arr = array();
                                foreach($file as $ff) {
                                    $txt = readTxt("$ff.txt");
                                    $arr[(int) explode("_", str_replace("../file/people/staff/$folder_name/", "", $ff), 2)[0]] = array("name"=>array_shift($txt),"file"=>$ff,"text"=>$txt);
                                }
                                        ...
                                        <div class="card mb-3 w-100">
                                            ...
                                            <div style="display: none;" id="targetEditZone_<?php echo $rndid;?>">
                                                <form method="POST" action="../endpoint/people_IO.php?function=update&index=<?php echo $currIndex; ?>&id=<?php echo $rndid; ?>&target=<?php echo $fi; ?>" enctype="multipart/form-data">
                                                    <input type="hidden" name="path" value="../file/people/staff/<?php echo $folder_name; ?>/">
