...
<body>
    <div class="container">
        <!-- Change Value here -->
        <h1 class="font-weight-bold text-md">ตารางแพทย์ออกตรวจนอกเวลา</h1><?php $category = "OPD"; ?>
        <?php if ($id == 0) {?>
        <div class="row">
            ...
            if ($stmt = $conn->prepare("SELECT `id`,`title` FROM `timetable` WHERE `category` = ?")) {
                ...
                if ($result->num_rows > 0) {
                    while ($row = $result->fetch_assoc()) { ?>
                        <a href="../timetable/<?php echo $category;?>-<?php echo $row['id']; ?>" class="md-dark">
                        ...
                        if ($result->num_rows > 0) {
                            while ($row = $result->fetch_assoc()) { ?>
                                <h4 class="font-weight-bold">
                                    <?php echo $row['title']; ?>
                                    <?php if ($isAdmin) { ?>
                                    <a class='z-depth-0 btn-sm btn-warning btn-floating mr-0 ml-0 mb-0 mt-0'href="../timetable/<?php echo $category;?>-<?php echo $id; ?>-edit"><i class="fas fa-pencil-alt"></i></a>
                                ...
                                <div class="card mb-3 w-100">
                                    <div class="card-body">
                                        <p class="card-text" id="articleRenderer"><?php print_r($row['data']); ?></p>
                                    </div>
                                </div>
                            <?php } ?>
                        <?php } ?>
