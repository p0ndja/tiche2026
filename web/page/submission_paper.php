            ...
            <div class="col-12 col-lg-9">
                ...
                <?php
                    if (!isAdmin()) {
                        ...
                        if ($stmt = $conn->prepare("SELECT COUNT(*) FROM paper_submission WHERE user_id = $userId")) {
                            $stmt->execute();
                            $stmt->bind_result($count);
                            $stmt->fetch();
                            if ($count > 0) {
                                echo '<div class="alert alert-info">You have already submitted a paper. See <a href="/submission-paper/list">[all your submission]</a>.</div>';
                            }
                            $stmt->close();
                        }
                    }
