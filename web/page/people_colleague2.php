...
<body>
    <div class="container-fluid px-5">
        <h1 class="font-weight-bold text-md">รายชื่อแพทย์ประจำบ้านต่อยอด</h1>
        <div class="row mb-3">
            <div class="col-md-3">
                <div class="card mb-3">
                    <ul class="nav md-pills pills-success flex-column" role="tablist">
                        <?php
                            $fol = glob("../file/people/colleague2/*", GLOB_ONLYDIR);
                            ...
                            <script>
                                function createNewCategory() {
                                    ...
                                    if (folderName != null && folderName != "") {
                                        window.location = "../endpoint/people_IO.php?function=create&top=colleague2&mkdir=" + folderName;
                                    }
                                }
                ...
                <div class="tab-content vertical">
                    <?php for($f = 0; $f < count($fol); $f++) {
                        ...
                        <div class="tab-pane fade in show <?php if ($f == 0) echo 'active'; ?>" id="<?php echo str_replace(" ", "_", pathinfo($fol[$f], PATHINFO_FILENAME)); ?>" role="tabpanel">
                            <?php $hotlink = "";
                            if (file_exists("../file/people/colleague2/$folder_name/link.txt")) {
                                $link = "";
                                foreach(readTxt("../file/people/colleague2/$folder_name/link.txt") as $l) {
                                    $link .= $l;
                                }
                                if (!empty($link) || isAdmin()) 
                                    $hotlink = "<a class=\"md\" ".(isAdmin() ? "href=\"#\" onclick='myEditLinkFunction(\"../file/people/colleague2/$folder_name\",\"".$link."\");'" : "href=\"".$link."\"")."><i class=\"fa-solid fa-link\"></i></a>";
                            } else if (isAdmin()) {
                                $hotlink = "<a class=\"md\" href=\"#\" onclick=\"myEditLinkFunction('../file/people/colleague2/$folder_name','');\"><i class=\"fa-solid fa-link\"></i></a>";
                            }?>
                            <h4 class="font-weight-bold"><?php echo $folder_name . " " . $hotlink; ?></h4>
                            <div class="row">
                            <?php
                                $file = glob("../file/people/colleague2/$folder_name/*.{jpg,png,gif,PNG,JPG,GIF,JPEG,jpeg}", GLOB_BRACE);
                                $arr = array();
                                foreach($file as $ff) {
                                    $txt = readTxt("$ff.txt");
                                    $arr[(int) explode("_", str_replace("../file/people/colleague2/$folder_name/", "", $ff), 2)[0]] = array("name"=>array_shift($txt),"file"=>$ff,"text"=>$txt);
                                }
                                for($currIndex = 1; $currIndex <= count($arr); $currIndex++) {
                                    ...
                                    <div class="col-12 col-xl-6 d-flex align-items-stretch">
                                        <div class="card mb-3 w-100">
                                            <div class="row no-gutters" id="targetZone_<?php echo $file_name;?>">
                                            ...
                                            <div style="display: none;" id="targetEditZone_<?php echo $file_name;?>">
                                                <form method="POST" action="../endpoint/people_IO.php?function=update&id=<?php echo $currIndex; ?>&target=<?php echo $fi; ?>" enctype="multipart/form-data">
                                                    <input type="hidden" name="path" value="../file/people/colleague2/<?php echo $folder_name; ?>/">
                                                    ...
                                                    <div class="row no-gutters">
                                                        <div class="col-5">
                                                            <div class="view overlay">
                                                                ...
                                                                <img src="<?php echo $fi; ?>" class="card-img-top" loading="lazy" id="demoImg_<?php echo $currIndex; ?>" style="width: 100%; height:300px; object-fit: cover; object-position: top;"/>
                                                        ...
                                                        <div class="col-7">
                                                            <div class="card-body">
                                                                ...
                                                                <textarea rows="8" class="form-control" name="description_<?php echo $currIndex; ?>" id="description_<?php echo $currIndex; ?>" placeholder="คำอธิบาย" style="resize:none; max-height: 220px;"><?php for($d = 0; $d < count($description); $d++) { echo $description[$d]; } ?></textarea>
                                                            </div>
                                                        </div>
                                                    </div>
                                                </form>
