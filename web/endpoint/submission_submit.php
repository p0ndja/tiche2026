<?php 
    ...
    if (
        ...
        header("Location: /submission/abstract");
        die();
    }

    $id = latestIncrement($db["table"],"submission");
    ...
    $sub_abstract = xss_clean($_POST['sub_abstract']);
    $sub_category = xss_clean($_POST['sub_category']);
    $sub_type = xss_clean($_POST['sub_type']);
    ...
    // remove the not allow symbol in the file name
    $sub_file_path = "../file/submission/abstract-$id-$sub_file_name";
        ...
        if (move_uploaded_file($sub_file_tmp, $sub_file_path)) {
            if ($stmt = $conn->prepare("INSERT INTO submission (
                ...
                if (!$stmt->execute()) {
                    ...
                    sendEmail(
                        $sub_email,
                        "TIChE2026 Abstract Submit Confirmation - ID#$id",
                        "https://tiche2026.ubu.ac.th/static/function/mail/template/submission_success.html",
                        array("name"=>$sub_fullName, "submissionId"=>$id, "date"=>date("Y-m-d H:i:s", time())));
                    sendEmail(
                        $sub_co_email,
                        "TIChE2026 Abstract Submit Confirmation - ID#$id",
                        "https://tiche2026.ubu.ac.th/static/function/mail/template/submission_success.html",
                        array("name"=>$sub_co_fullName, "submissionId"=>$id, "date"=>date("Y-m-d H:i:s", time())));
                    $conn->close();
