<?php 
    ...
    if (isset($_POST['reg_category']) && !in_array(xss_clean($_POST['reg_category']), array("Presenter", "Participant", "Senior", "High-School"))) {
        ...
        if ($reg_category == "Presenter" || $reg_category == "Participant") {
        ...
        } else if ($reg_category == "High-School") {
            if (
                empty($_POST['reg_code']) ||
                empty($_POST['reg_seniorProject_projectName']) ||
                empty($_POST['reg_seniorProject_primaryContactName']) ||
            ...
            $reg_fullName = xss_clean($_POST['reg_seniorProject_primaryContactName']);
            $reg_affiliation = xss_clean($_POST['reg_seniorProject_projectName']);
            $reg_email = xss_clean($_POST['reg_seniorProject_primaryContactEmail']);
            $reg_phone = xss_clean($_POST['reg_highSchoolAttendee_number']);
            $reg_code = xss_clean($_POST['reg_code']);
            $reg_note = xss_clean($_POST['reg_note']);

            $price = 3750 + (1250 * intval($reg_phone)); // 3750 for primary contact + 1250 for each extra participant
